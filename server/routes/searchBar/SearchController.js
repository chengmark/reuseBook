    ...
    if (dbs){
        ...
        for (let i=0;i<dbs.length;i++)
        {
           ...
           // console.log(title + " " + title.length + " " + cnt);
            ...
            ans.push(dbs[i],cnt/title.length);
        }
        ...
        for (let i=0;i<dbs.length;i++)
        {
           ...
           //console.log(title + " " + title.length + " " + cnt);
            if (cnt>title.length/3)
            {
                ...
                await bookDB.find({categoryId: dbs[i].id})
                .then(data => {
...
function jaro_winkler(s1,s2){
   // console.log(s1+ " -- " + s2);
  //  console.log("jw1");
    if (s1==s2 || (s2.includes(s1) && s1.length>=s2.length/3))
    return 1;
    let l1=s1.length;
    let l2=s2.length;
    if (!l1 || !l2)
   ...
   // console.log("jw2");
    let m=0,t=0,dj,range = Math.abs(Math.floor(Math.max(l1,l2)/2)-1),match1=new Array(s1.length),match2=new Array(s2.length);
    for (let i=0;i<l1;i++)
    {
